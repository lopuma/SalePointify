  ...
  return (
    <Container {...props}>
      <header
        ...
        role="alert"
      >
        <span className="font-medium">Warning!</span> The client should not
        repeat this request without modification.
      </header>
      <main
        className={cn(
          'relative z-10 bg-primary py-[120px] text-white/90',
          className
        )}
      >
...
export function ErrorValidation({ message, className, ...props }) {
  const defaultErrorValidation =
    'text-xs text-never-foreground dark:text-never italic'
  return (
    message && (
